...
import { activateNotebooksExtension, openAndShowWorkspaceNotebook } from './util.js';
...
suite('notebookExport', () => {
	...
	test('shows empty quick pick when no exporters are registered', async () => {
		...
		await vscode.commands.executeCommand(Command.ExportNotebook, notebook.uri);
	...
	test('shows only exporters that support the notebook language', async () => {
		...
		const notebook = await openAndShowWorkspaceNotebook('test-notebook.ipynb');
		const api = await activateNotebooksExtension();
