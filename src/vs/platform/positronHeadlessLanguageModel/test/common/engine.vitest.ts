...
describe('HeadlessLanguageModelEngineChannelClient', () => {
	it('reassembles delta chunks into a text stream and ends on done (R1)', async () => {
	...
	it('cancellation ends the stream and tears down the channel subscription (R4)', async () => {
		const channel = new FakeChannel();
		const client = new HeadlessLanguageModelEngineChannelClient(channel);
