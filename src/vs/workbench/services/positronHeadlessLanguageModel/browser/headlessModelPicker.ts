...
/**
 * A consistent, reusable model picker (R9): "use the default" plus the
...
export async function showHeadlessModelPicker(
	...
	const userValue = configurationService.inspect<string[]>(options.settingKey).userValue;
	const usingDefault = !userValue || userValue.length === 0;
	const pinnedId = userValue && userValue.length === 1 ? userValue[0] : undefined;
	...
	const byVendor = new Map<string, { id: string; name: string }[]>();
	for (const model of models) {
		const group = byVendor.get(model.vendor) ?? [];
		group.push({ id: model.id, name: model.name });
